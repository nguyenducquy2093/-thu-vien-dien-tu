...
// Multer/busboy decode tên file multipart theo latin1 mặc định, làm hỏng
// tên file có dấu tiếng Việt (VD: "HỌC" -> "Há»ŒC"). Chuyển lại đúng utf8 ngay khi nhận file.
...
const memoryUpload = multer({
  ...
  fileFilter: (req, file, cb) => {
    file.originalname = fixVietnameseFilename(file.originalname);
...
// setHeaders: ép Content-Disposition inline để trình duyệt hiển thị PDF/Office
// trực tiếp trong iframe, không tự tải xuống.
...
app.get('/api/books/:id/read', async (req, res) => {
  ...
  if (book.fileName) {
    ...
    // File không còn trên đĩa (thường do server Render restart và mất ổ đĩa tạm).
    console.error('[MISSING FILE] Sách id=' + book.id + ' "' + book.title + '" thiếu file tại ' + fpath + ' (originalName: ' + book.originalName + ')');
    return res.status(404).json({ success: false, message: 'File đã bị mất trên server (có thể do server khởi động lại). Vui lòng upload lại tài liệu.' });
  }
